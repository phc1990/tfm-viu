...
Fits performed
--------------
C0  PRIMARY: field PSF, all valid pixels.
C1  STAR-CORE MASKED: same model, but masks the central cores of A1 and A2.
    This tests whether the asteroid flux is constrained by the unobscured trail
    rather than by how the model divides photons in the overlap region.
C2  SOURCE-PSF SENSITIVITY: trail uses field PSF, while A1/A2 use their own
    SRCLIST FWHM/PA values.  This is diagnostic only.
...
def fit_scene(cut, xx, yy, trail_template, star_templates, scene_origin_xy,
              bkg_rms_ref, core_mask_centres=None, core_radius=0.0):
...
def main() -> None:
    ...
    # Primary all-pixel fit.
    fit0 = fit_scene(cut, xx, yy, trail_t, star_t_field, origin, bkg_rms_ref)

    # Core-masked validation fit.
    fit1 = fit_scene(
        cut, xx, yy, trail_t, star_t_field, origin, bkg_rms_ref,
        core_mask_centres=star_xy, core_radius=STAR_CORE_MASK_RADIUS_PIX,
    )

    # Source-specific star PSFs as sensitivity diagnostic; trail remains field PSF.
    star_t_src = []
    source_psf_info = []
    ...
    fit2 = fit_scene(cut, xx, yy, trail_t, star_t_src, origin, bkg_rms_ref)
    ...
    der1 = derive_fit(fit1, apfrac, exptime, manual_ref)
    ...
    def print_fit(label, fit, der):
        ...
        for i, s in enumerate(der["stars"], start=1):
            print(f"A{i} total joint-fit rate         : {s['total_rate']:.6f} +/- {s['total_rate_err_formal']:.6f} ct/s")
            print(f"A{i} rate inside h13-Horizons     : {s['rate_in_h13_horizons_rect']:.6f} ct/s")
    ...
    print_fit("C0 FIELD-PSF JOINT FIT [PRIMARY]", fit0, der0)
    print_fit(f"C1 STAR-CORE MASKED r<{STAR_CORE_MASK_RADIUS_PIX:.1f}px [VALIDATION]", fit1, der1)
    print_fit("C2 SOURCE-SPECIFIC STAR PSFs [SENSITIVITY]", fit2, der2)

    print("\n--- Stability diagnostics ---")
    print(f"masked - primary Delta R_h       : {der1['trail_Rh_h13_horizons_rect'] - der0['trail_Rh_h13_horizons_rect']:+.9f} ct/s")
    ...
    np.savez_compressed(
        ...
        masked_model=fit1["model"],
        masked_residual=fit1["residual"],
        masked_fit_mask=fit1["fit_mask"],
    ...
    result = {
        ...
        "fit_star_core_masked": {"fit": serialisable_fit(fit1), "derived": der1},
        ...
        "method_notes": [
            ...
            "The star-core-masked fit tests whether asteroid flux is constrained by unobscured trail pixels rather than the overlap cores.",
    ...
    print("  (1) Inspect the residual image for a coherent remaining trail or point-source core.")
    print("  (2) Inspect corr(trail,A1), corr(trail,A2), corr(A1,A2); large |corr| means degeneracy.")
    print("  (3) Compare PRIMARY vs STAR-CORE MASKED R_h.  This is the key Weisell robustness test.")
    print("  (4) Compare field-PSF vs source-specific-PSF sensitivity.")
    print("  (5) Closure against the nearest-width manual h=13 extraction is diagnostic only.")
